...
export const getOverview = async (req, res, next) => {
    try {
        ...
        const [totalTasks, inProgressTasks, blockedTasks, readyForReviewTasks, overdueTasks, liveTasksRaw, recentActivity, teamWorkloadRaw, teams] = await Promise.all([
        ...
        const workloadRaw = teamWorkloadRaw
            ...
            if (!teamInfo)
                return null;
            if (t._count.id > maxWorkload)
                maxWorkload = t._count.id;
            return {
                id: teamInfo.id,
                name: teamInfo.name,
        ...
        const formattedRecentActivity = recentActivity.map(log => ({
            ...log,
            target: log.metadata?.title || 'مهمة'
        }));
