...
export const getActivityLogs = async (filters, authScope) => {
    const page = Math.max(1, filters.page || 1);
    const limit = Math.min(100, Math.max(1, filters.limit || 50));
    const skip = (page - 1) * limit;
    // Build WHERE clause
    const where = { ...authScope };
    ...
    rawSummary.forEach(s => {
        if (s.action.includes('TASK_'))
            totalTasks += s._count.id;
        if (s.action.includes('_CREATED'))
            totalCreates += s._count.id;
        if (s.action === 'TASK_STATUS_CHANGED' || s.action === 'TASK_BLOCKED' || s.action === 'TASK_UNBLOCKED')
            totalStatus += s._count.id;
        if (s.action.includes('REVIEW'))
    ...
    const formattedItems = items.map(item => ({
        ...
        entity: {
            id: item.entityId,
            title: item.metadata?.title || 'عنصر النظام', // Assumes metadata stores title of the task
