...
const buildDateRange = (filters: ReportFilters) => {
  const now = new Date();
  ...
  switch (filters.period) {
    ...
    case 'last_90_days':
      ...
      break;
    case 'custom':
      if (filters.from) from = startOfDay(new Date(filters.from));
      if (filters.to) to = endOfDay(new Date(filters.to));
      break;
    default:
...
export const generateOperationalReport = async (filters: ReportFilters, user: any) => {
  const scope = getAuthorizedTeamScope(user, filters.teamId);
  if (!scope) throw Object.assign(new Error('Unauthorized scope'), { code: 'FORBIDDEN' });
  ...
  if (totalTasks > 0) {
    ...
    if (overdueTasks / totalTasks > 0.15) {
      ...
      if (healthStatus !== 'Critical') healthStatus = 'Needs Attention';
  ...
  tasksForTrend.forEach(t => {
    ...
    if (!trendMap[cDate]) trendMap[cDate] = { created: 0, completed: 0 };
    ...
    if (t.status === 'completed') {
      ...
      if (!trendMap[uDate]) trendMap[uDate] = { created: 0, completed: 0 };
  ...
  const teamWorkloadMap: Record<string, any> = {};
  ...
  teamWorkloadRaw.forEach(agg => {
    ...
    if (!teamWorkloadMap[tid]) return; // Fallback
    const count = agg._count.id;
    teamWorkloadMap[tid].total += count;
    if (agg.status === 'completed') teamWorkloadMap[tid].completed += count;
    else if (agg.status === 'in_progress') teamWorkloadMap[tid].active += count;
    else if (agg.status === 'blocked') teamWorkloadMap[tid].blocked += count;
  });
