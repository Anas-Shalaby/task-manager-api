...
export const getTasks = async (filters: any, page: number, limit: number, user: any) => {
  ...
  if (filters.sort) {
    if (filters.sort === 'deadline') orderBy = { deadline: 'asc' };
    else if (filters.sort === 'priority') orderBy = { priority: 'desc' };
    // else keep default
  }
...
export const getTaskMetrics = async (user: any, filters: any = {}) => {
  const where: any = {};
...
export const fetchMyTasks = async (user: any, filters: any, page: number, limit: number) => {
  const skip = (page - 1) * limit;
  ...
  if (filters.sort) {
    ...
    else if (filters.sort === 'createdAt') orderBy = { createdAt: 'desc' };
  }
...
export const fetchMyTaskMetrics = async (user: any) => {
  const where: any = {
    assignees: { some: { userId: user.id } }
  };
...
export const getTaskById = async (id: string, user: any) => {
  ...
  if (task && !canViewTask(user, task)) {
    const error: any = new Error('ليس لديك صلاحية لتنفيذ هذا الإجراء');
...
export const createTask = async (data: any, user: any) => {
  ...
  const task = await prisma.$transaction(async (tx) => {
    ...
    if (assignees && assignees.length > 0) {
      ...
      const notifyUsers = assignees.filter((a: any) => a.userId !== userId);
      if (notifyUsers.length > 0) {
        notificationsToEmit = await createManyNotifications(notifyUsers.map((a: any) => ({
          userId: a.userId,
          type: 'TASK_ASSIGNED',
          title: 'مهمة جديدة',
...
export const updateTask = async (id: string, data: any, user: any) => {
  ...
  if (!existingTask) throw Object.assign(new Error('المهمة غير موجودة'), { code: 'NOT_FOUND' });
  ...
  if (!isOwner && !isManager) throw Object.assign(new Error('ليس لديك صلاحية لتعديل هذه المهمة'), { code: 'FORBIDDEN' });
  ...
  const task = await prisma.$transaction(async (tx) => {
    ...
    const changes: any = {};
    if (title && title !== existingTask.title) changes.title = { from: existingTask.title, to: title };
    if (priority && priority !== existingTask.priority) changes.priority = { from: existingTask.priority, to: priority };
...
export const deleteTask = async (id: string, user: any) => {
  ...
  const isOwner = existingTask.assignees.some(a => a.userId === user.id && a.isOwner);
  const isManager = ['admin', 'manager', 'supervisor'].includes(user.role);
  if (!isOwner && !isManager) throw Object.assign(new Error('ليس لديك صلاحية لمسح هذه المهمة'), { code: 'FORBIDDEN' });
...
const executeTransition = async (id: string, user: any, targetStatus: string, validateProgressFn?: (t: any) => void, extraTxLogic?: (tx: any, notificationsToEmit: any[]) => Promise<void>) => {
  const existingTask = await prisma.task.findUnique({
    where: { id },
    include: { assignees: true }
  });
  ...
  if (!existingTask) {
    const error: any = new Error('المهمة غير موجودة');
    error.code = 'NOT_FOUND';
    throw error;
  }

  if (!canTransitionTask(existingTask, existingTask.status, targetStatus, user)) {
    const error: any = new Error(`لا يمكن تغيير الحالة من ${existingTask.status} إلى ${targetStatus}`);
    error.code = 'CONFLICT';
    throw error;
  }
  ...
  const task = await prisma.$transaction(async (tx) => {
    ...
    const changes: any = { status: { from: existingTask.status, to: targetStatus } };
    if ((progressUpdate as any).progress && existingTask.progress !== 100) {
...
export const startTask = (id: string, user: any) => {
  return executeTransition(id, user, 'in_progress');
};

export const blockTask = (id: string, reason: string, user: any) => {
  return executeTransition(id, user, 'blocked', undefined, async (tx, notificationsToEmit) => {
    ...
    const owner = task?.assignees.find((a: any) => a.isOwner);
...
export const unblockTask = (id: string, user: any) => {
...
export const submitForReview = (id: string, user: any) => {
...
export const reviewTask = (id: string, approved: boolean, feedback: string | undefined, user: any) => {
...
export const addOperationalUpdate = async (id: string, data: { message: string, progress?: number, status?: string }, user: any) => {
  const existingTask = await prisma.task.findUnique({
    where: { id },
    include: { assignees: true }
  });
  ...
  const isManager = ['admin', 'manager', 'supervisor'].includes(user.role);
  if (!isAssigned && !isManager) throw Object.assign(new Error('ليس لديك صلاحية'), { code: 'FORBIDDEN' });
  ...
  const task = await prisma.$transaction(async (tx) => {
    ...
    if (newProgress !== existingTask.progress || newStatus !== existingTask.status) {
      const changes: any = {};
      if (newProgress !== existingTask.progress) changes.progress = { from: existingTask.progress, to: newProgress };
      if (newStatus !== existingTask.status) changes.status = { from: existingTask.status, to: newStatus };
...
export const addComment = async (id: string, message: string, user: any) => {
  const existingTask = await prisma.task.findUnique({
    where: { id },
    include: { assignees: true }
  });
  ...
  if (!existingTask) throw Object.assign(new Error('المهمة غير موجودة'), { code: 'NOT_FOUND' });
  if (!canViewTask(user, existingTask)) throw Object.assign(new Error('ليس لديك صلاحية'), { code: 'FORBIDDEN' });

  const comment = await prisma.taskComment.create({
    data: { taskId: id, userId: user.id, message }
  });

  const participantsToNotify = existingTask.assignees.filter((a: any) => a.userId !== user.id);
  if (participantsToNotify.length > 0) {
    const notifications = await createManyNotifications(participantsToNotify.map((a: any) => ({
      userId: a.userId,
      type: 'TASK_COMMENTED',
      title: 'تعليق جديد',
